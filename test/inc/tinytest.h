...
void 		tt_execute(const char* name, void (*test_function)())
{
	...
	{
		printf("%c%sfailure:%c%s %s:%d: In test: %s():\n    %s (%s)\n",
	...
	{
		printf("%c%ssucess:%c%s %s:%d: In test: %s()\n",
			TT_COLOR_CODE, TT_COLOR_GREEN, TT_COLOR_CODE, TT_COLOR_RESET,
			tt_current_file, tt_current_line, name);
		tt_passes++;
	}
}
...
int 		tt_report(void)
{
	if (tt_fails)
	{
		printf("\n%c%sFAILED%c%s [%s] (passed:%c%s%d%c%s, failed:%c%s%d%c%s, total:%c%s%d%c%s)\n",
		TT_COLOR_CODE, TT_COLOR_RED, TT_COLOR_CODE, TT_COLOR_RESET,
		tt_current_file,
		TT_COLOR_CODE, TT_COLOR_GREEN, tt_passes, TT_COLOR_CODE, TT_COLOR_RESET,
		TT_COLOR_CODE, TT_COLOR_RED, tt_fails, TT_COLOR_CODE, TT_COLOR_RESET,
		TT_COLOR_CODE, TT_COLOR_RED, tt_passes + tt_fails, TT_COLOR_CODE, TT_COLOR_RESET);
		return -1;
	}
	else
	{
		printf("\n%c%sSUCESS%c%s [%s] (passed:%c%s%d%c%s, failed:%c%s%d%c%s, total:%c%s%d%c%s)\n",
		TT_COLOR_CODE, TT_COLOR_GREEN, TT_COLOR_CODE, TT_COLOR_RESET,
		tt_current_file,
		TT_COLOR_CODE, TT_COLOR_GREEN, tt_passes, TT_COLOR_CODE, TT_COLOR_RESET,
		TT_COLOR_CODE, TT_COLOR_RED, tt_fails, TT_COLOR_CODE, TT_COLOR_RESET,
		TT_COLOR_CODE, TT_COLOR_GREEN, tt_passes + tt_fails, TT_COLOR_CODE, TT_COLOR_RESET);
		return 0;
	}
}
